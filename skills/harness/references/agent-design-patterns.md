...
**모델:** 에이전트의 역할과 업무 복잡도에 따라 모델을 선택한다.
...
## Context 로딩 (작업 시작 시 필수)
1. `context/` 디렉토리 내 모든 `.md` 파일을 읽는다.
2. 특히 다음 내용을 참조한다 (파일명 변경 시에도 디렉토리 스캔으로 자동 탐지):
   - BRAND-VOICE: 톤, 언어 정책, 금지 패턴
   - WORKING-RULES: MUST/SHOULD 규칙, 출력 규칙
   - GLOSSARY: 도메인 용어 정의
   - ABOUT-ME: 팀 정보, 커뮤니케이션 선호도
3. 작업 중 위 규칙과 충돌 시 즉시 보고하고 수정한다.
