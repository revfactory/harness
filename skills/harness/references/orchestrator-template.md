...
### Phase 0: 컨텍스트 확인 및 환경 감지

기존 산출물 존재 여부를 확인하고, 실행 환경(OS)에 맞는 도구를 선택한다.

1. **OS 감지 및 도구 선택**:
   - Windows: `$env:OS` 확인 → PowerShell (`Get-ChildItem`) 또는 `Read` 도구 + 절대 경로 사용
   - Unix/macOS: `uname` 확인 → Bash (`ls`, `find`) 사용
   - **권장:** 파일 읽기는 `Read` 도구를 직접 사용 (OS 독립적). 디렉토리 스캔만 OS별 도구로 분기.
2. `_workspace/` 디렉토리 존재 여부 확인 (OS에 맞는 도구로)
3. 실행 모드 결정:
...
4. 부분 재실행 시: 이전 산출물 경로를 에이전트 프롬프트에 포함하여, 에이전트가 기존 결과를 읽고 피드백을 반영하도록 지시

### Phase 1: 준비
1. 사용자 입력 분석 — {무엇을 파악하는지}
2. **Context 로딩** — `context/` 디렉토리 내 모든 `.md` 파일을 읽어 에이전트 프롬프트에 포함
   - **디렉토리 스캔 (권장):**
     - Windows: `Get-ChildItem -Path .\context -Filter *.md` (PowerShell)
     - Unix: `ls context/*.md` (Bash)
   - **OS 독립적 방법 (Fallback):** `Read` 도구로 파일명을 하나씩 직접 읽기
   - 읽은 내용을 에이전트 지시에 포함
3. 작업 디렉토리에 `_workspace/` 생성
   - **초기 실행**: 새 `_workspace/` 생성
   - **새 실행**: 기존 `_workspace/`를 `_workspace_{YYYYMMDD_HHMMSS}/`로 이동한 직후 새 `_workspace/` 재생성
4. 입력 데이터를 `_workspace/00_input/`에 저장

### Phase 2: 팀 구성

> **Fallback:** `TeamCreate`를 사용할 수 없는 경우, Phase 2를 건너뛰고 Phase 3을 "단일 세션 순차 실행"으로 대체한다.
> 각 "팀원" 역할을 오케스트레이터가 직접 수행하며, `_workspace/`에 `{phase}_{agent}_{artifact}.md` 형식으로 중간 산출물을 기록한다.
...
1. 팀 생성:
   ...
   TeamCreate(
     ...
     members: [
       { name: "{teammate-1}", agent_type: "{type}", model: "sonnet", prompt: "{역할 설명 및 작업 지시}" },
       { name: "{teammate-2}", agent_type: "{type}", model: "sonnet", prompt: "{역할 설명 및 작업 지시}" },
       ...
     ]
   )
   ```
...
3. **독립 검증** — 별도의 검증 에이전트를 서브 에이전트로 호출:
   - 입력: 모든 산출물 파일 경로, `context/` 내 BRAND-VOICE 및 WORKING-RULES
   - 검증 항목: 브랜드 톤 준수, 출력 규칙 준수, 데이터 출처 명시, 키워드 일관성
   - 출력: `_workspace/` 하위 qa_report 파일
4. **판정 결과 처리**:
   - **PASS** → Phase 5로 진행
   - **FIX** → 수정 지시를 `_workspace/` 하위 파일로 저장 → `SendMessage`로 해당 팀원에게 수정 지시 전달 (최대 2회) → 3으로 돌아가 재검증
     - 재검증 시 이전 qa_report와 비교하여 개선 여부 확인
     - 동일한 FIX가 2회 반복되면 REJECT로 전환
   - **REJECT** → 사용자에게 보고, 수동 개입 요청
5. {통합/검증 로직}
6. 최종 산출물 생성: `{output-path}/{filename}`

### Phase 5: 후처리 및 정리

> **핵심 원칙:** 이 Phase는 텍스트로 "갱신했다"고 보고해서는 안 된다. 반드시 `Write`/`Edit` 도구로 파일을 직접 수정하고, `Read` 도구로 확인해야 한다.

1. **ROADMAP.md 갱신** (필수 — `Edit` 도구 사용):
   - 완료된 작업의 체크박스를 `[ ]` → `[x]`로 변경
   - 새로 알게 된 제약사항이나 비목표가 있으면 추가
   - 갱신 후 `Read` 도구로 파일 내용 확인
2. **BRIEF.md 갱신** (필수 — `Edit` 도구 사용):
   - 세션 목표 달성 여부를 각 항목 옆에 `(완료)` 또는 `(미완료: 사유)` 기록
   - 미완료 항목은 HANDOFF.md의 "다음 세션 할 일"에 반영
3. **context/LESSONS.md 갱신** (필수 — `Edit` 도구 사용):
   - "변경 이력" 테이블에 새 행 추가: `| {오늘 날짜} | {작업 내용} | {대상 파일/에이전트} | {사유} |`
   - "학습 사항" 섹션에 새 항목 추가 (Date, Context, Lesson, Action)
   - 갱신 후 `Read` 도구로 파일 내용 확인
4. **HANDOFF.md 갱신** (필수 — `Write` 도구 사용):
   - "이번 세션" 섹션: 날짜, 범위, 상태, 완료된 작업(3개 이상 구체적 나열), 결정 사항
   - "다음 세션" 섹션: 할 일(2개 이상), 차단 요소, 참고 사항
   - 갱신 후 `Read` 도구로 파일 내용 확인
5. **팀원들에게 종료 요청 (SendMessage)**
6. **팀 정리 (TeamDelete)**
7. **`_workspace/` 디렉토리 보존** (중간 산출물은 삭제하지 않음 — 사후 검증·감사 추적용)
8. **사용자에게 결과 요약 보고**
9. **세션 갱신 완료 체크리스트 확인**:
   ```
   - [ ] ROADMAP.md에 완료된 작업 체크 및 진행 상황 갱신 완료
   - [ ] BRIEF.md에 세션 목표 달성 여부 기록 완료
   - [ ] LESSONS.md에 변경 이력 또는 학습 사항 추가 완료
   - [ ] HANDOFF.md에 이번 세션 완료 작업, 결정 사항, 다음 세션 할일 기록 완료
   ```
   - 미체크 항목이 있으면 사용자에게 "세션 관리 파일 갱신이 완료되지 않았습니다: {항목}" 경고
10. "세션 관리 파일 갱신을 완료했습니다. 검토가 필요하신가요?" 라고 사용자에게 질의
...
|---------|------|------|-------|-------------------|
| {agent-1} | {소스} | `_workspace/{phase}_{agent}_{artifact}.md` | sonnet | true |
| {agent-2} | {소스} | `_workspace/{phase}_{agent}_{artifact}.md` | sonnet | true |
