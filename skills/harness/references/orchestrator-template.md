...
2. **Context 로딩** — `context/` 디렉토리 내 모든 `.md` 파일을 읽어 에이전트 프롬프트에 포함
   - 파일명을 하드코딩하지 않고 `context/` 디렉토리 스캔으로 자동 탐지
   - BRAND-VOICE, WORKING-RULES, GLOSSARY, ABOUT-ME 등의 내용을 에이전트 지시에 포함
...
4. 검증 결과를 바탕으로 필요 시 팀원에게 수정 요청 (SendMessage)
5. {통합/검증 로직}
6. 최종 산출물 생성: `{output-path}/{filename}`

### Phase 5: 후처리 및 정리
1. **문서 업데이트**:
   - `ROADMAP.md`: 완료된 작업 체크 및 진행 상황 갱신
   - `BRIEF.md`: 세션 목표 달성 여부 기록
   - `context/LESSONS.md`: 학습 사항 기록 (Date, Context, Lesson, Action)
   - `HANDOFF.md`: 다음 세션을 위한 연결 정보 작성
2. 팀원들에게 종료 요청 (SendMessage)
3. 팀 정리 (TeamDelete)
4. `_workspace/` 디렉토리 보존 (중간 산출물은 삭제하지 않음 — 사후 검증·감사 추적용)
5. 사용자에게 결과 요약 보고
6. "LESSONS.md와 HANDOFF.md를 업데이트했습니다. 검토가 필요하신가요?" 라고 사용자에게 질의
