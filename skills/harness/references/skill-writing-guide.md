...
생성-검증 루프에서 검증 에이전트가 실제로 품질을 개선하도록 보장하는 규칙.

### 9-1. 판정 체계

검증 에이전트는 3단계 판정을 사용한다:

| 판정 | 의미 | 후속 조치 |
|------|------|----------|
| **PASS** | 품질 기준 충족 | Phase 5(종료)로 진행 |
| **FIX** | 수정 필요 | 구체적 수정 지시 작성 → 해당 에이전트 재호출 → 재검증 |
| **REJECT** | 기준 미달, 재작성 필요 | 사용자에게 보고, 수동 개입 요청 |

### 9-2. FIX 판정 시 동작 규칙

1. **구체적 수정 지시 작성**: "수정하라"가 아닌, bullet point로 정확한 수정 내용 명시
   - "2장 3절의 코드 예시에 언어 태그 `yaml` 추가"
   - "도입부 '상상해보세요' → '예를 들어'로 변경 (BRAND-VOICE 금지 패턴)"
2. **직접 파일 수정**: `Edit` 또는 `Write` 도구로 파일을 직접 수정. 텍스트로 "수정했다"고 보고 금지.
3. **변경 여부 확인**: 수정 전후 파일 해시 비교 또는 `diff`로 실제 변경 확인
4. **최대 재시도**: 2회 (무한 루프 방지). 2회 초과 시 REJECT로 전환

### 9-3. PASS 판정 조건

- "수정 사항 0개"가 아닌 **"품질 기준 충족"**으로 정의
- 검증 기준은 WORKING-RULES와 BRAND-VOICE에 명시된 항목을 따름
- 모호한 경우 REJECT보다 FIX 선택 (보수적 판단)
