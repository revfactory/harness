...
6. **선택적 권장 패턴 적용 여부 판단** — 도메인 특성에 따라 다음을 검토한다 (default는 적용 안 함):
   - 장시간 작업·세션 재개·자동 컴팩트가 필요한 도메인이면 `references/memo-pattern.md`의 **분산 슬롯 + 공유 헤더** 패턴 적용을 고려
...
- **Memo 패턴 (선택)**: `references/memo-pattern.md` — 분산 슬롯 + 공유 헤더로 컨텍스트를 영속화하면서도 에이전트 독립성을 유지하는 토큰 효율 패턴. 적용 기준, 디렉토리 구조, 갱신 정책, TaskCreate/SendMessage/MEMORY/_workspace와의 역할 분리표 포함.
