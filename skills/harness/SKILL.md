...
**핵심 원칙:**
1. `/harness` 호출 시 프로젝트 루트에 표준 컨텍스트 구조(`context/`, `BRIEF.md`, `HANDOFF.md`, `ROADMAP.md`, `CLAUDE.md`)를 자동 생성한다.
2. 에이전트 정의(`.claude/agents/`)와 스킬(`.claude/skills/`)을 생성한다.
3. **에이전트 팀을 기본 실행 모드로 사용한다.**
4. **CLAUDE.md에 하네스 포인터를 등록한다.** — 새 세션에서 오케스트레이터 스킬이 트리거되도록 최소한의 포인터(트리거 규칙 + 변경 이력)만 기록한다. 추가로 **context 참조 규칙**을 포함하여 에이전트/스킬이 자동으로 컨텍스트를 활용하도록 유도한다.
5. **하네스는 고정물이 아니라 진화하는 시스템이다.** — 매 실행 후 피드백을 반영하고, 에이전트·스킬·CLAUDE.md를 지속 갱신한다.
...
1. `프로젝트/.claude/agents/`, `프로젝트/.claude/skills/`, `프로젝트/CLAUDE.md` 존재 여부 확인
2. `context/`, `BRIEF.md`, `HANDOFF.md`, `ROADMAP.md` 존재 여부 확인
3. 현황에 따라 실행 모드를 분기한다:
   ...
   - **운영/유지보수**: 기존 하네스의 감사·수정·동기화 요청 → Phase 8-5 운영/유지보수 워크플로우로 이동

   **기존 확장 시 Phase 선택 매트릭스:**
   | 변경 유형 | Phase 1 | Phase 2 | Phase 3 | Phase 4 | Phase 5 | Phase 6 | Phase 7 |
   |----------|---------|---------|---------|---------|---------|---------|---------|
   | 에이전트 추가 | 건너뜀 | 건너뜀 | 배치 결정만 | 필수 | 전용 스킬 필요 시 | 오케스트레이터 수정 | 필수 |
   | 스킬 추가/수정 | 건너뜀 | 건너뜀 | 건너뜀 | 건너뜀 | 필수 | 연결 변경 시 | 필수 |
   | 아키텍처 변경 | 건너뜀 | 건너뜀 | 필수 | 영향받는 에이전트만 | 영향받는 스킬만 | 필수 | 필수 |
4. 기존 에이전트/스킬 목록과 CLAUDE.md 기록을 대조하여 불일치(drift)를 감지한다
5. 감사 결과를 사용자에게 요약 보고하고, 실행 계획을 확인받는다

### Phase 1: 컨텍스트 초기화

**신규 구축 시에만 실행.** 기존 확장/유지보수 시에는 건너뜀.

1. `context/` 폴더 및 파일들 생성 (없으면)
   - `ABOUT-ME.md`: 사용자 신원, 역할, 커뮤니케이션 선호도
   - `BRAND-VOICE.md`: 톤, 언어, 금지 패턴
   - `WORKING-RULES.md`: MUST/SHOULD 규칙
   - `GLOSSARY.md`: 도메인 용어집
   - `LESSONS.md`: 학습 사항 기록
   - 템플릿은 `references/context-writing-templates.md` 참조
2. 세션 관리 파일 생성 (템플릿 기반)
   - `BRIEF.md` — 이번 세션의 구체적 목표 (ROADMAP의 한 조각)
   - `HANDOFF.md` — 세션 간 연결 (완료 작업 / 다음 세션 할 일)
   - `ROADMAP.md` — 장기 계획 및 전체 작업 방향
3. `CLAUDE.md` 생성/업데이트 — 워크스페이스 헌법 + **context 참조 규칙** 포함:
   ```markdown
   ## Context 참조 규칙
   ...
   ```
4. 사용자에게 "context 파일들을 자신의 상황에 맞게 채워달라"고 안내
...
6. **Context 파일 자동 채우기** — Phase 1에서 생성된 context 파일들이 템플릿 상태이면, 도메인 분석 결과를 바탕으로 자동으로 채운다:
   - `context/ABOUT-ME.md`: 사용자 역할, 도메인, 커뮤니케이션 선호도 (대화에서 파악한 정보 기반)
   - `context/BRAND-VOICE.md`: 톤, 언어 정책 (사용자 대화 스타일 반영)
   - `context/WORKING-RULES.md`: 도메인별 MUST/SHOULD 규칙
   - `context/GLOSSARY.md`: 도메인 핵심 용어 정의
   - `BRIEF.md`: 현재 하네스 구축 목표
   - 이미 채워진 파일은 건너뛰고, 템플릿 상태만 자동 채움
...
> **현재 제약:** Claude Code의 `TeamCreate` 도구는 일부 환경에서 비활성화되어 있을 수 있습니다.
> `TeamCreate`를 사용할 수 없는 경우:
> 1. **단일 세션 파이프라인 시뮬레이션**: 오케스트레이터가 직접 각 Phase를 순차 실행하며 `_workspace/`에 중간 산출물을 기록
> 2. **Agent 도구 + run_in_background**: 병렬 작업이 필요한 Phase에서 `Agent` 도구를 `run_in_background: true`로 호출
> 3. **하이브리드**: Phase 2(병렬 수집)는 Agent 도구, Phase 3(통합)은 단일 세션
> 실행 모드 선택 시 이 제약을 고려하여 설계하라.
...
1. 작업을 전문 영역으로 분해
2. 에이전트 팀 구조 결정 (아키텍처 패턴은 `references/agent-design-patterns.md` 참조, 템플릿은 `team-architecture-templates/` 참조)
   - **파이프라인**: 순차 의존 작업
   - **팬아웃/팬인**: 병렬 독립 작업
   - **전문가 풀**: 상황별 선택 호출
   - **생성-검증**: 생성 후 품질 검수
...
**모델 설정:** 에이전트의 역할과 업무 복잡도에 따라 모델을 선택한다.

| 모델 | 사용 기준 | 예시 역할 |
|------|----------|----------|
| `haiku` | 정해진 규칙대로 수행하는 단순 업무, 오타/형식 검수 등 반복적 작업 | 스타일 가이드 검수, 단순 포맷팅 |
| `sonnet` (기본) | 대부분의 일반적 업무. 기본값이며 별도 명시 없이도 안정적인 결과 | 코드 리뷰, 문서 작성, 데이터 추출, 일반 분석 |
| `opus` | 아키텍처 설계, 복잡한 추론, 고품질 창작물 등 고난이도 업무 | 시스템 설계, 복잡한 디버깅, 창작적 기획 |

Agent 도구 호출 시 `model` 파라미터를 명시한다. 기본값은 `sonnet`이다. `sonnet`으로 수행 시 품질이 부족하다고 판단되면 `opus`로 대체하여 재시도할 수 있다.
...
#### 5-4. Progressive Disclosure (단계적 정보 공개)
...
### Phase 6: 통합 및 오케스트레이션

오케스트레이터는 스킬의 특수한 형태로, 개별 에이전트와 스킬을 하나의 워크플로우로 엮어 팀 전체를 조율한다. Phase 5에서 생성한 개별 스킬이 "각 에이전트가 무엇을 어떻게 하는가"를 정의한다면, 오케스트레이터는 "누가 언제 어떤 순서로 협업하는가"를 정의한다. 구체적 템플릿은 `references/orchestrator-template.md` 참조.
...
Phase 3-1에서 선택한 실행 모드에 따라 오케스트레이터 패턴이 달라진다:
...
1. **세션 시작 시**: `context/` 디렉토리 내 모든 `.md` 파일 읽기 (파일명 하드코딩 금지, 디렉토리 스캔으로 자동 탐지)
2. **작업 수행 전**: `BRIEF.md` + `context/WORKING-RULES.md` 읽기
3. **도메인 용어 확인**: `context/GLOSSARY.md` 참조
4. **작업 완료 후**: `context/LESSONS.md`에 학습 사항 기록, `HANDOFF.md` 업데이트, `ROADMAP.md` 진행 상황 갱신
5. **검증 규칙**: 작업 완료 후 독립 검증 에이전트를 호출하여 산출물 품질 검증
````

**CLAUDE.md에 넣지 않는 것:** 에이전트 목록, 스킬 목록, 디렉토리 구조, 실행 규칙 상세, 변경 이력. 이유: 에이전트/스킬 목록은 오케스트레이터 스킬과 `.claude/agents/`, `.claude/skills/`에서 관리하므로 중복이다. 디렉토리 구조는 파일 시스템에서 직접 확인 가능하다. 변경 이력은 `context/LESSONS.md`에 기록한다. CLAUDE.md는 **포인터(트리거 규칙) + context 참조 규칙**만 담는다.
...
- 에이전트 간 참조 일관성 확인
- 커맨드가 생성되지 않았는지 확인
- **context 파일들이 올바른 위치에 있는지 확인** (`context/`, `BRIEF.md`, `HANDOFF.md`, `ROADMAP.md`)
- **CLAUDE.md에 context 참조 규칙이 포함되어 있는지 확인**
...
| 컨텍스트 누락 | context 파일 | "브랜드 톤이 반영 안 됨" → BRAND-VOICE.md 업데이트 유도 |

#### 8-3. 변경 이력

모든 변경은 `context/LESSONS.md`의 **변경 이력** 테이블에 기록한다. 변경 이력의 핵심은 **언제, 무엇을, 왜** 변경했는지 추적하여 퇴행(regression)을 방지하는 것이다.

**LESSONS.md 변경 이력 테이블 형식:**
...
추가로 상세한 학습 사항은 같은 파일의 **학습 사항** 섹션에 기록한다 (Date, Context, Lesson, Action 형식).
...
- `context/` 파일 존재 여부 및 최신성 확인
- 감사 결과를 사용자에게 보고한다

**Step 2: 점진적 추가/수정**
- 사용자 요청에 따라 에이전트 추가/수정/삭제, 스킬 추가/수정/삭제, context 파일 업데이트를 수행한다
- 변경은 한 번에 하나씩, 각 변경 후 즉시 Step 3(동기화)을 실행한다

**Step 3: LESSONS.md 변경 이력 갱신**
- `context/LESSONS.md`의 변경 이력 테이블에 변경 사항을 추가한다 (날짜, 변경 내용, 대상, 사유)

**Step 4: 변경 검증**
- 수정된 에이전트/스킬의 구조 검증 (Phase 7-1 기준)
- 수정 범위가 트리거에 영향을 주면 트리거 검증 (Phase 7-4 기준)
- 대규모 변경(아키텍처 변경, 에이전트 3개 이상 추가/삭제) 시 Phase 7-3(실행 테스트), 7-5(드라이런)까지 수행
...
- [ ] `context/` — **ABOUT-ME.md, BRAND-VOICE.md, WORKING-RULES.md, GLOSSARY.md, LESSONS.md**
- [ ] `BRIEF.md`, `HANDOFF.md`, `ROADMAP.md` — **세션 관리 파일**
- [ ] `CLAUDE.md` — **워크스페이스 헌법 + context 참조 규칙 + 하네스 포인터**
...
- [ ] Agent 호출 시 `model` 파라미터 명시 — 기본 sonnet, 단순 업무는 haiku, 복잡한 설계/추론은 opus
...
- [ ] **CLAUDE.md에 하네스 포인터 등록** (트리거 규칙 + context 참조 규칙)
- [ ] **context/LESSONS.md에 변경 이력 테이블 기록** — 에이전트/스킬/컨텍스트 추가/삭제/수정 사항 (날짜, 변경 내용, 대상, 사유)
- [ ] **오케스트레이터 Phase 0에 컨텍스트 확인 단계** (초기/후속/부분 재실행 판별)
- [ ] **오케스트레이터 Phase 1에 `_workspace/` 생성 로직 포함** (초기 실행: 신규 생성, 새 실행: 기존 폴더를 `_workspace_{timestamp}/`로 이동 후 재생성)
- [ ] **오케스트레이터 Phase 5에 세션 관리 파일 갱신 강제화** — `Write`/`Edit` 도구로 직접 수정 후 `Read` 도구로 확인
  - [ ] `HANDOFF.md`에 이번 세션 완료 작업(3개 이상), 결정 사항, 다음 세션 할일 기록
  - [ ] `ROADMAP.md`에 완료된 작업 체크 및 진행 상황 갱신
  - [ ] `LESSONS.md`에 변경 이력 또는 학습 사항 추가
  - [ ] `BRIEF.md`에 세션 목표 달성 여부 기록
- [ ] **오케스트레이터 종료 시 세션 갱신 완료 체크리스트 확인** — 미완료 항목 있으면 사용자 경고
...
- **컨텍스트 시스템 가이드**: `references/context-system-guide.md` — 컨텍스트 자동 초기화 구조, 참조 규칙, outputs 규칙
- **컨텍스트 작성 템플릿**: `references/context-writing-templates.md` — context/ 파일 작성 예시/가이드
- **팀 아키텍처 템플릿**: `team-architecture-templates/*.md` — 6대 패턴별 베이스 팀 템플릿