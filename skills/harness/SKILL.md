...
하네스의 실행 모드를 먼저 결정한다. **기본값은 에이전트 팀**이다.

| 기준 | 에이전트 팀 (기본) | 서브 에이전트 |
|------|-------------------|-------------|
| **에이전트 간 통신** | 직접 메시지 (SendMessage) | 메인에게만 결과 보고 |
| **조율 방식** | 공유 작업 목록 (TaskCreate) + 자체 조율 | 메인 에이전트가 모든 작업 관리 |
| **컨텍스트** | 각 팀원이 독립 컨텍스트 윈도우 | 결과가 호출자에게 요약 반환 |
| **토큰 비용** | 높음 (각각 별도 Claude 인스턴스) | 낮음 |
| **적합한 작업** | 논의·협업·도전이 필요한 복잡한 작업 | 결과만 중요한 집중된 단발 작업 |

**에이전트 팀을 선택하라 (강력 권장 — 기본값):**
- 2개 이상의 에이전트가 작업할 때는 **에이전트 팀을 우선 고려**한다
- 에이전트 간 커뮤니케이션이 활발할수록 결과 품질이 높아진다 — 팀원들이 서로의 발견을 공유하고, 상충되는 정보를 토론하고, 누락된 관점을 보완하는 과정이 핵심 가치
- 리서치·분석 작업에서도 에이전트 팀이 서브 에이전트보다 우월한 결과를 산출한다 (한 에이전트가 발견한 정보가 다른 에이전트의 조사 방향을 실시간으로 수정할 수 있음)

**서브 에이전트가 적합한 경우:**
- 에이전트가 1개뿐인 단발성 작업 (파일 탐색, 단순 코드 분석 등)
- 에이전트 간 통신이 불필요한 구조 (결과만 중요하고 과정 중 협업이 불필요)
- 생성-검증 패턴에서 에이전트가 2개뿐이고 결과 전달만 필요한 경우
- 전문가 풀 패턴에서 상황별로 1개 에이전트만 선택 호출하는 경우

> **원칙:** 에이전트 팀이 기본이다. 서브 에이전트를 선택할 때는 "팀원 간 통신이 정말 불필요한가?"를 자문한다. 통신이 필요하다면 팀 모드가 맞다.
...
각 에이전트를 `프로젝트/.claude/agents/{name}.md`에 정의한다:
...
**에이전트 팀 모드에서의 추가 고려사항:**
- 에이전트 정의에 `## 팀 통신 프로토콜` 섹션을 추가하여 어떤 팀원에게 어떤 메시지를 보내는지 명시
- 공유 작업 목록에서 자체적으로 작업을 요청(claim)할 수 있도록 작업 범위를 명확히 정의
- 다른 팀원의 결과에 대한 검증/도전 역할이 있으면 명시
...
오케스트레이터 내에 에러 처리 방침을 포함한다:

| 에러 유형 | 전략 | 구현 |
|----------|------|------|
| **에이전트 실패** | 재시도 → 스킵 → 폴백 | 최대 1회 재시도. 재실패 시 해당 에이전트 결과 없이 진행하되 보고서에 누락 명시 |
| **부분 실패** | 가용 결과로 진행 | 4개 중 3개 성공이면 3개 결과로 종합. 1개 이하면 사용자에게 알림 |
| **타임아웃** | 현재까지 수집된 결과 사용 | 에이전트에 timeout 명시, 초과 시 부분 결과라도 출력 |
| **데이터 충돌** | 출처 명시 후 병기 | 에이전트 간 상충 정보는 삭제하지 않고 출처와 함께 병기 |
| **팀원 중지** (팀 모드) | 리더가 감지 후 재생성 | 팀원이 에러로 중지 시 리더에게 자동 알림. 대체 팀원 생성 또는 작업 재할당 |
